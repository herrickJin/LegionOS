...
| `agent-svc`    | 数字员工域             | 原型与运行配置定义中心，实例和任务分配由inf-svc管理             | PostgreSQL（JSONB存原型配置）+ Redis（原型配置缓存）                          |
...
| `agent-svc`     | 数字员工域 | 同步REST（供inf-svc读取原型配置） | PostgreSQL + Redis      |
...
| `/api/v1/agent-prototypes` | GET | 查询原型列表（按技能/模型筛选） | 复合型研发经理 |
| `/api/v1/agent-prototypes/{id}` | GET | 查询单个原型详情 | 复合型研发经理、inf-svc（内部调用） |
| `/api/v1/agent-prototypes/{id}` | PUT | 更新原型配置 | AI Agent架构师 |
| `/api/v1/agent-prototypes/{id}/runtime-config` | GET | 获取原型运行配置（供 inf-svc 实例化时读取） | inf-svc（内部调用） |

**发布事件**（RabbitMQ）：

| 事件名 | 触发场景 | 消费方 | 投递保证 |
|--------|---------|--------|---------|
| `Agent.PrototypeUpdated` | 原型配置更新 | 基础设施域（`inf-svc`，刷新实例缓存） | at-least-once |

**端口**：8081（HTTP）、9091（Actuator）

**数据存储**：
- PostgreSQL 16+（JSONB存原型配置、能力定义、运行配置）
- Redis（原型配置缓存，供 inf-svc 高频读取）

**关键设计**：
- agent-svc 是原型和配置的定义中心，不持有运行时状态
- inf-svc 实例化时通过 Feign 读取 agent-svc 的原型配置
- 原型配置更新时通过事件通知 inf-svc 刷新缓存
...
| `/api/v1/users/{id}/endpoint` | PATCH | 登记数字员工通信地址（inf-svc 部署成功后回调） | 系统服务（inf-svc） |
...
**职责边界**：
- 数字员工实例生命周期管理（实例化/启动/暂停/销毁，编排 iam-svc 注册 + agent-svc 配置读取）
- 数字员工Body运行时管理（启动/停止/监控）
- 任务分配（技能匹配、负载均衡、实例调度，由 pdm-svc 触发）
...
| 接口路径 | 方法 | 职责 | 权限要求 |
|----------|------|------|----------|
| `/api/v1/agent-instances` | POST | 实例化数字员工（三域协同编排） | 复合型研发经理 |
| `/api/v1/agent-instances/{id}` | GET | 查询实例状态 | 复合型研发经理 |
| `/api/v1/agent-instances` | GET | 查询实例列表（按项目/状态筛选） | 复合型研发经理 |
| `/api/v1/agent-instances/{id}/start` | POST | 启动实例（部署Body并建立WebSocket） | 复合型研发经理 |
| `/api/v1/agent-instances/{id}/stop` | POST | 停止实例（销毁Body） | 复合型研发经理 |
| `/api/v1/agent-instances/{id}/assign-task` | POST | 分配任务给数字员工（技能匹配+实例选择+WebSocket推送） | pdm-svc（内部调用） |
| `/api/v1/bodies/{id}` | GET | 查询Body运行载体状态 | 系统管理员 |
| `/ws/v1/agent/{instanceId}` | WebSocket | Server-ACP通信 | 数字员工实例（JWT认证） |
| `/api/v1/llm/generate` | POST | AI生成接口（LLM路由+Token计量） | pdm-svc（内部调用） |
| `/api/v1/llm/quota/{instanceId}` | GET | 查询Token配额状态 | 复合型研发经理 |
| `/api/v1/models` | GET | 查询可用模型列表 | 复合型研发经理 |
| `/api/v1/contexts` | POST | 创建项目上下文 | 复合型研发经理 |
| `/api/v1/nodes` | GET | 查询运行节点状态 | 系统管理员 |
| `/api/v1/adapters/gitlab/*` | ALL | GitLab适配器代理 | pdm-svc（内部调用） |

**发布事件**（RabbitMQ）：

| 事件名 | 触发场景 | 消费方 | 投递保证 |
|--------|---------|--------|---------|
| `Agent.InstanceCreated` | 数字员工实例创建成功 | 项目实施域（`pdm-svc`，刷新资源视图） | at-least-once |
| `Agent.InstanceDeployed` | 数字员工Body部署成功，通信地址就绪 | 用户权限域（`iam-svc`，登记地址） | at-least-once |
| `Agent.InstanceStopped` | 实例停止（手动/配额耗尽/异常） | 项目实施域（`pdm-svc`，释放资源分配） | at-least-once |
| `Token.QuotaExceeded` | Token配额耗尽 | 项目实施域（`pdm-svc`，触发配额预警） | at-least-once |

**消费事件**（RabbitMQ）：

| 事件名 | 来源 | 处理逻辑 |
|--------|------|---------|
| `Agent.TaskAssigned` | 项目实施域（`pdm-svc`） | 技能匹配、选择实例、通过WebSocket推送给对应数字员工实例 |
...
**4. pdm-svc → inf-svc（同步REST，任务分配）**
- `pdm-svc` 同步调用 `inf-svc` 的 `/api/v1/agent-instances/{id}/assign-task` 接口
- `inf-svc` 内部完成技能匹配、实例选择、WebSocket推送
- 同步调用保证任务分配实时性，失败则抛出异常触发事务回滚

**5. 数字员工实例部署启动（三域协同，inf-svc 编排）**

数字员工实例的部署入口在 `inf-svc`，由 inf-svc 编排三域协同：

1. **复合型研发经理**调用 `inf-svc` 的 `POST /api/v1/agent-instances`，指定原型和项目
2. **inf-svc** 读取 agent-svc 的原型配置（技能、模型、运行参数）
3. **inf-svc** 调用 iam-svc 注册数字员工用户，申请工号（user_id）
4. **inf-svc** 根据原型配置 + 节点资源部署 Body 运行时
5. **inf-svc** 向 iam-svc 回调登记数字员工实例的通信地址

调用链：
- `前端/调用方` → `inf-svc`：触发实例化
- `inf-svc` → `agent-svc`：读取原型配置
- `inf-svc` → `iam-svc`：注册数字员工，获取工号
- `inf-svc` → `iam-svc`：汇报数字员工地址，完成登记

**6. pdm-svc → inf-svc（OpenFeign，任务分配+AI能力）**
- 任务分配时，`pdm-svc`调用`inf-svc`的`/api/v1/agent-instances/{id}/assign-task`接口（inf-svc 内部完成技能匹配+实例选择+WebSocket推送）
- AI能力调用时，`pdm-svc`调用`inf-svc`的`/api/v1/llm/generate`接口
- 调用链：`pdm-svc` → `inf-svc` → WebSocket（推送任务）/ LLM API（生成内容）

**7. 所有服务 → iam-svc（OpenFeign）**
...
- `inf-svc`接收结果后，通过事件或回调通知`pdm-svc`
...
2. **第2个月**：完成`workflow-svc`（任务类型、工作流定义）+ `agent-svc`（原型定义）+ `inf-svc`（实例生命周期管理、任务分配）
...
flowchart LR
  ...
  AI -->|instance_id| EL["execution_log<br/>执行日志"]
  AI -->|instance_id| CH["conversation_history<br/>对话历史"]
  B -.读取模型配置.-> M["model<br/>模型注册表"]
  B -.调度到.-> N["runtime_node<br/>运行节点"]
```

**中文解释**：
- 这张图描述的是 `inf-svc` 如何管理数字员工的运行实例和实际运行资源。
- `agent_instance` 是数字员工的运行实例记录，记录它来自哪个原型、绑定哪个项目、当前状态和配额。通过 `prototype_id` 跨库关联 agent-svc 的 `agent_prototype`。
- `body` 是实例真正运行时的载体，可以理解为容器化后的执行单元。每个 `agent_instance` 最多对应一个活跃 `body`。
- `execution_log` 记录实例执行过程中的关键行为、日志和 Token 消耗。
- `conversation_history` 保存实例的对话上下文，便于追溯数字员工在任务执行中的思考与交互过程。
- `model` 负责登记可用模型及其配置，`body` 运行时会读取对应模型信息。
- `runtime_node` 表示底层计算节点，`body` 会被调度到具体节点上运行。

**核心表摘要**：

| 表名 | 业务定位 | 主键/唯一约束 | 核心字段 | 关联关系 |
|------|----------|---------------|----------|----------|
| `agent_instance` | 数字员工运行实例 | `id`, `instance_id` | `prototype_id`, `project_id`, `status`, `assigned_tasks`, `runtime_config_override`, `token_quota` | 通过 `prototype_id` 跨库关联 `agent-svc.agent_prototype`，下挂 body、日志与对话历史 |
| `execution_log` | 执行日志 | `id`, `log_id` | `instance_id`, `task_id`, `node_execution_id`, `level`, `message`, `details` | 归属于 `agent_instance` |
| `conversation_history` | 会话历史 | `id`, `conversation_id` | `instance_id`, `task_id`, `messages` | 归属于 `agent_instance` |
...
**JSONB/数组字段设计重点**：
- `agent_instance.runtime_config_override`：仅保存实例级覆盖项，避免复制整份原型配置（原型配置从 agent-svc 的 `agent_prototype.runtime_config` 读取）。
- `agent_instance.token_quota`：存总额度、已用额度、剩余额度，便于成本治理和配额熔断。
- `agent_instance.assigned_tasks`：当前分配的任务ID列表，支持任务分配查询。
- `execution_log.details`：半结构化日志数据（工具调用参数、返回结果、耗时），支持审计和回溯。
- `conversation_history.messages`：对话消息数组（角色、内容、时间戳），支持上下文检索。
...
**索引与查询策略**：
- GIN 索引覆盖 `runtime_config_override`、`assigned_tasks`、`details`、`messages`、`config`、`health_check`、`spec`、`cost`、`capabilities` 等 JSONB/数组字段。
- B-tree 索引覆盖 `prototype_id`、`project_id`、`status`、`instance_id`、`provider`、`timestamp` 等高频条件字段。
- 部分索引聚焦 `RUNNING` 状态的 instance 和 body、`HEALTHY` 的节点，优化在线查询和调度器查询。

**典型查询场景**：
- 按 `prototype_id` 查询某原型下的所有运行实例。
- 按项目、状态筛选运行中的数字员工实例。
- 按任务或节点追溯一次执行过程中的日志和会话历史。
- 查询具备 GPU 的节点或 GPU 使用率超过阈值的节点。
- 查询某个实例当前运行在哪个 Body 上。
...
- `inf:instance:{instance_id}:status`：实例状态缓存（TTL 60秒），减少高频状态查询打库
- `inf:instance:{instance_id}:token:used`：Token实时消耗计数器
- `inf:ws:session:{instance_id}`：WebSocket会话映射（实例ID→连接ID）
...
| `pdm-svc` | `inf-svc` | `/api/v1/agent-instances/{id}/assign-task` | 分配任务给数字员工（inf-svc负责技能匹配+推送） | 连接5s，读取30s |
| `pdm-svc` | `inf-svc` | `/api/v1/llm/generate` | 调用LLM生成接口 | 连接5s，读取60s |
| `pdm-svc` | `iam-svc` | `/api/v1/auth/verify` | 验证JWT有效性 | 连接2s，读取5s |
| `pdm-svc` | `iam-svc` | `/api/v1/auth/check-permission` | 校验用户功能权限 | 连接2s，读取5s |
| `inf-svc` | `agent-svc` | `/api/v1/agent-prototypes/{id}/runtime-config` | 实例化时读取原型运行配置 | 连接3s，读取5s |
| `inf-svc` | `iam-svc` | `/api/v1/users` | 注册数字员工用户、登记通信地址 | 连接5s，读取10s |
...
pdm-svc (核心业务服务)
    ...
    ▼
inf-svc (基础设施服务)
    │ 8. 查询数字员工实例状态（本地 agent_instance 表）
    │ 9. 技能匹配，选择目标实例
    │ 10. 读取 agent-svc 原型配置（缓存优先）
...
| `Agent.InstanceCreated` | 基础设施域（`inf-svc`） | 项目实施域（`pdm-svc`，刷新资源视图） | `instance_id`, `prototype_id` | at-least-once |
| `Agent.InstanceDeployed` | 基础设施域（`inf-svc`） | 用户权限域（`iam-svc`，登记通信地址） | `instance_id`, `user_id`, `endpoint` | at-least-once |
| `Agent.TaskAssigned` | 项目实施域（`pdm-svc`） | 基础设施域→数字员工实例（inf-svc WebSocket转发） | `instance_id`, `task_id`, `task_spec` | WebSocket推送 |
| `Agent.TaskCompleted` | 基础设施域（`inf-svc`） | 项目实施域（`pdm-svc`） | `instance_id`, `task_id`, `result`, `evidence` | at-least-once |
...
Server（`inf-svc`）与数字员工实例（ACP）之间通过**WebSocket**通信，协议名为**Server-ACP**。任务由 `pdm-svc` 触发，`inf-svc` 负责技能匹配、实例选择并通过 WebSocket 推送。
